...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  if (HAL_GPIO_ReadPin(GPIOA, input0) == GPIO_PIN_SET)
	      {
		  HAL_GPIO_WritePin(GPIOB, led0, GPIO_PIN_SET);
	      }
	  if (HAL_GPIO_ReadPin(GPIOB, input1) == GPIO_PIN_SET)
	  	  {
	  		  HAL_GPIO_WritePin(GPIOA, led1, GPIO_PIN_SET);
	  	  }
	  if (HAL_GPIO_ReadPin(GPIOB, input2) == GPIO_PIN_SET)
	  	  {
	  		  HAL_GPIO_WritePin(GPIOA, led2, GPIO_PIN_SET);
	  	  }
	  if (HAL_GPIO_ReadPin(GPIOA, input3) == GPIO_PIN_SET)
	  	  {
